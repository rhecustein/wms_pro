{{-- resources/views/inventory/pallets/create.blade.php --}}
@extends('layouts.app')

@section('title', 'Create New Pallet')
...
<div class="container-fluid px-4 py-6">
    ...
    <div class="mb-6">
        ...
        <p class="text-sm text-gray-600 mt-1">Add a new pallet to inventory</p>
    </div>

    <form action="{{ route('inventory.pallets.store') }}" method="POST">
        @csrf
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="pallet_type" required class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 @error('pallet_type') border-red-500 @enderror">
                                ...
                                <option value="standard" {{ old('pallet_type') === 'standard' ? 'selected' : '' }}>Standard</option>
                                <option value="euro" {{ old('pallet_type') === 'euro' ? 'selected' : '' }}>Euro</option>
                                <option value="custom" {{ old('pallet_type') === 'custom' ? 'selected' : '' }}>Custom</option>
                            </select>
                            @error('pallet_type')
                        ...
                        <div>
                            ...
                            <select name="storage_bin_id" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 @error('storage_bin_id') border-red-500 @enderror">
                                ...
                                @foreach($storageBins as $bin)
                                    <option value="{{ $bin->id }}" {{ old('storage_bin_id') == $bin->id ? 'selected' : '' }}>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="number" name="width_cm" step="0.01" value="{{ old('width_cm', 120) }}" required class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 @error('width_cm') border-red-500 @enderror">
                            @error('width_cm')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="depth_cm" step="0.01" value="{{ old('depth_cm', 120) }}" required class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 @error('depth_cm') border-red-500 @enderror">
                            @error('depth_cm')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="height_cm" step="0.01" value="{{ old('height_cm', 16) }}" required class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 @error('height_cm') border-red-500 @enderror">
                            @error('height_cm')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="max_weight_kg" step="0.01" value="{{ old('max_weight_kg', 1200) }}" required class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 @error('max_weight_kg') border-red-500 @enderror">
                            @error('max_weight_kg')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="barcode" value="{{ old('barcode') }}" placeholder="Auto-generated if empty" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 @error('barcode') border-red-500 @enderror">
                        ...
                        <div>
                            ...
                            <input type="text" name="qr_code" value="{{ old('qr_code') }}" placeholder="Auto-generated if empty" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 @error('qr_code') border-red-500 @enderror">
                            <p class="text-xs text-gray-500 mt-1">Leave empty for auto-generation</p>
                            @error('qr_code')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <textarea name="notes" rows="4" placeholder="Add any additional notes..." class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 @error('notes') border-red-500 @enderror">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div class="space-y-2">
                        <button type="submit" class="w-full px-4 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-medium">
                            <i class="fas fa-save mr-2"></i>Create Pallet
                        </button>
                        <a href="{{ route('inventory.pallets.index') }}" class="block w-full px-4 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition text-center font-medium">
