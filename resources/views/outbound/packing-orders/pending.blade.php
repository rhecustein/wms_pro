...
@section('title', 'Pending Packing Orders')
...
<div class="container-fluid px-4 py-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-600 mt-1">Orders waiting to be packed</p>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-yellow-600 mt-2">{{ $packingOrders->total() }}</p>
                </div>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-orange-600 mt-2">
                        {{ $packingOrders->where('assigned_to', null)->count() }}
                    </p>
                </div>
                <div class="w-12 h-12 bg-orange-100 rounded-lg flex items-center justify-center">
                    <i class="fas fa-user-slash text-2xl text-orange-600"></i>
                </div>
            </div>
        </div>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-blue-600 mt-2">
                        {{ $packingOrders->whereNotNull('assigned_to')->count() }}
                    </p>
                </div>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-green-600 mt-2">
                        {{ $packingOrders->where('packing_date', '>=', today())->where('packing_date', '<', today()->addDay())->count() }}
                    </p>
                </div>
                    ...
                    @forelse($packingOrders as $order)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4">
                                ...
                                @else
                                    <button onclick="openAssignModal({{ $order->id }}, '{{ $order->packing_number }}')" class="text-sm text-orange-600 hover:text-orange-800 font-medium">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @php
                                    $ageInHours = $order->created_at->diffInHours(now());
                                    $colorClass = $ageInHours > 24 ? 'text-red-600' : ($ageInHours > 12 ? 'text-orange-600' : 'text-green-600');
                                @endphp
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-gray-600 mb-4">There are no pending packing orders at the moment</p>
                                    <a href="{{ route('outbound.packing-orders.index') }}" class="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
    ...
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-xl bg-white">
        ...
        <form id="assignForm" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="assignOrderNumber" class="w-full rounded-lg border-gray-300 bg-gray-100" readonly>
            </div>
            ...
            <div class="flex space-x-3">
                ...
                <button type="button" onclick="closeAssignModal()" class="flex-1 px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition">
                    Cancel
                </button>
            </div>
        </form>
...
<script>
    function openAssignModal(orderId, orderNumber) {
        ...
        document.getElementById('assignForm').action = `/outbound/packing-orders/${orderId}/assign`;
