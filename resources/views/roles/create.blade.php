...
<div class="container-fluid px-4 py-6 max-w-4xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-600 mt-1">Add a new role with specific permissions</p>
        </div>
        ...
        <form action="{{ route('roles.store') }}" method="POST">
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" required class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 @error('name') border-red-500 @enderror" placeholder="e.g., Administrator, Manager, Staff">
                    @error('name')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-xs text-gray-500 mt-1">Slug will be auto-generated from the role name</p>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 @error('description') border-red-500 @enderror" placeholder="Brief description of this role and its responsibilities">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    @foreach($modulePermissions as $key => $label)
                                        <label class="flex items-center p-3 bg-white border border-gray-200 rounded-lg hover:bg-blue-50 hover:border-blue-300 transition cursor-pointer">
                                            <input type="checkbox" name="permissions[]" value="{{ $key }}" {{ in_array($key, old('permissions', [])) ? 'checked' : '' }} class="permission-checkbox {{ strtolower($module) }}-permission rounded border-gray-300 text-blue-600 focus:ring-blue-500 mr-3">
                                            <div class="flex-1">
