...
@section('title', 'Edit Transfer Order')
...
<div class="container-fluid px-4 py-6">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex space-x-3">
            <a href="{{ route('operations.transfer-orders.show', $transferOrder) }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition">
    ...
    <form action="{{ route('operations.transfer-orders.update', $transferOrder) }}" method="POST" id="transferOrderForm">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" value="{{ $transferOrder->transfer_number }}" class="w-full rounded-lg border-gray-300 bg-gray-100" disabled>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="transfer_type" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500" required>
                                ...
                                <option value="inter_warehouse" {{ old('transfer_type', $transferOrder->transfer_type) === 'inter_warehouse' ? 'selected' : '' }}>Inter Warehouse</option>
                                <option value="internal_bin" {{ old('transfer_type', $transferOrder->transfer_type) === 'internal_bin' ? 'selected' : '' }}>Internal Bin</option>
                                <option value="consolidation" {{ old('transfer_type', $transferOrder->transfer_type) === 'consolidation' ? 'selected' : '' }}>Consolidation</option>
                            </select>
                            @error('transfer_type')
                        ...
                        <div>
                            ...
                            <select name="from_warehouse_id" id="from_warehouse_id" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500" required>
                                ...
                                @foreach($warehouses as $warehouse)
                                    <option value="{{ $warehouse->id }}" {{ old('from_warehouse_id', $transferOrder->from_warehouse_id) == $warehouse->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="to_warehouse_id" id="to_warehouse_id" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500" required>
                                <option value="">Select Warehouse</option>
                                @foreach($warehouses as $warehouse)
                                    <option value="{{ $warehouse->id }}" {{ old('to_warehouse_id', $transferOrder->to_warehouse_id) == $warehouse->id ? 'selected' : '' }}>
                                        {{ $warehouse->name }} ({{ $warehouse->code }})
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="transfer_date" value="{{ old('transfer_date', $transferOrder->transfer_date->format('Y-m-d\TH:i')) }}" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500" required>
                            @error('transfer_date')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="expected_arrival_date" value="{{ old('expected_arrival_date', $transferOrder->expected_arrival_date?->format('Y-m-d\TH:i')) }}" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500">
                        ...
                        <div>
                            ...
                            <select name="vehicle_id" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500">
                                ...
                                @foreach($vehicles as $vehicle)
                                    <option value="{{ $vehicle->id }}" {{ old('vehicle_id', $transferOrder->vehicle_id) == $vehicle->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="driver_id" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500">
                                ...
                                @foreach($drivers as $driver)
                                    <option value="{{ $driver->id }}" {{ old('driver_id', $transferOrder->driver_id) == $driver->id ? 'selected' : '' }}>
                                        {{ $driver->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="mt-6">
                        ...
                        <textarea name="notes" rows="3" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500" placeholder="Additional notes or instructions...">{{ old('notes', $transferOrder->notes) }}</textarea>
                        @error('notes')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div id="no-items-message" class="text-center py-12" style="display: none;">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="space-y-3">
                        <button type="submit" class="w-full px-4 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-semibold">
                            <i class="fas fa-save mr-2"></i>Update Transfer Order
                        </button>
                        
                        <a href="{{ route('operations.transfer-orders.show', $transferOrder) }}" class="w-full px-4 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition font-semibold inline-block text-center">
                ...
                <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-6">
                    ...
                    <p class="text-xs text-yellow-700">
                        Updating this transfer order will replace all existing items. Make sure to add all necessary items before saving.
                    </p>
                </div>
    ...
    <div class="item-row border border-gray-200 rounded-lg p-4 bg-gray-50">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" name="items[INDEX][from_storage_bin_id]" placeholder="Bin Code" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500">
            </div>
            
            {{-- To Storage Bin --}}
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">To Storage Bin</label>
                <input type="text" name="items[INDEX][to_storage_bin_id]" placeholder="Bin Code" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500">
            </div>
            
            {{-- Batch Number --}}
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Batch Number</label>
                <input type="text" name="items[INDEX][batch_number]" placeholder="Batch Number" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500">
            </div>
            
            {{-- Serial Number --}}
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Serial Number</label>
                <input type="text" name="items[INDEX][serial_number]" placeholder="Serial Number" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500">
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    const existingItems = @json($transferOrder->items);
